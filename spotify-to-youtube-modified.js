...
function SpotifyToYoutube (spotifyApi) {
  ...
  return async function spotifyToYoutube (urisOrIdsOrTracks, options = {}) {
    ...
    return Promise.all(urisOrIdsOrTracks.map(async track => {
      ...
      const songsAndVideos = content.filter(track => track.type === 'song' || track.type === 'video')// && !track.name.toLowerCase().includes("8-Bit"))

	  if (["Good Kid"].includes(track.artists[0].name)) {
